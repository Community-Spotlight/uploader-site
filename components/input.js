/**
 * VInput - a custom input element with automatic value casting and validation.
 * Supports types: text, email, password, number, range
 */
class VInput extends HTMLElement {
  ...
  /**
   * Gets the validated and cast value of the input.
   * Applies trimming, min/max constraints, pattern matching, and email validation.
  ...
  getValue(opt_fallbackValue) {
    ...
    switch (type) {
      ...
      case "range": {
        value = Number(value);
        if (this.hasAttribute("dontErrorClamp")) {
          if (isNaN(value)) value = null;
          if (max !== null && value > Number(max)) value = null;
          if (min !== null && value < Number(min)) value = null;
        } else {
